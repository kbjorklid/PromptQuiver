...
### Added
- **Comment Titles:** If a prompt starts with a comment (`--`) followed by an empty line, that comment is used as the title in the list view, and only the title is shown.

### Changed
- **Comment Stripping:** Improved `stripComments` to remove the leading empty line when a comment title is detected, resulting in cleaner clipboard content.
...
[0.4.0]: https://github.com/kbjorklid/PromptQuiver/compare/v0.3.0...v0.4.0
